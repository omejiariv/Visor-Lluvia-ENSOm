...
logo_path = "Cuenca Verde Logo_V1.JPG"
logo_gota_path = "Cuenca Verde GoticaLogo.JPG"
...
if 'data_loaded' not in st.session_state:
    ...
    st.session_state.gdf_stations, st.session_state.df_precip_anual, st.session_state.gdf_municipios, \
        st.session_state.df_long, st.session_state.df_enso = preprocess_data(uploaded_file_mapa, uploaded_file_precip, uploaded_zip_shapefile)
...
select_all_checkbox = st.sidebar.checkbox("Seleccionar/Deseleccionar todas las estaciones",
                                           value=st.session_state.select_all_stations_state, key='select_all_checkbox')
if select_all_checkbox:
    st.session_state.selected_stations_auto = stations_options
else:
    if st.session_state.select_all_stations_state and st.session_state.selected_stations_auto == stations_options:
        st.session_state.selected_stations_auto = []
    ...
    with sub_tab_anual:
        with st.expander("Ver Gráfico de Precipitación Anual", expanded=True):
            if not df_anual_melted.empty:
                ...
                st.info("No hay datos anuales para la selección actual.")
        ...
        if not df_monthly_filtered.empty:
            ...
            with st.expander("Ver Tabla de Datos Detallados"):
                ...
                if not df_monthly_filtered.empty:
                    ...
                    st.info("No hay datos mensuales para la selección actual.")
        else:
            st.info("No hay datos mensuales para la selección actual.")
...
with tab_anim:
    ...
    with st.expander("Ver Animación de Puntos", expanded=True):
        ...
        if not df_anual_melted.empty:
            fig_mapa_animado = px.scatter_geo(df_anual_melted, lat='latitud_geo', lon='longitud_geo',
                                              color='precipitacion', size='precipitacion', hover_name='nom_est', animation_frame='año',
            ...
            st.warning("No hay datos anuales para crear el mapa animado.")
                        ...
                        if len(data_year) < 3:
                            ...
                            with map_col1:
                                ...
                                fig1 = px.scatter_geo(data_year, lat='latitud_geo', lon='longitud_geo', color='precipitacion',
                                                      size='precipitacion', hover_name='nom_est', color_continuous_scale='YIGnBu',
                            ...
                            with map_col2, st.spinner("Generando mapa Kriging..."):
                                ...
                                fig2 = go.Figure(data=go.Contour(z=z, x=grid_lon, y=grid_lat, colorscale='YIGnBu', zmin=color_range[0], zmax=color_range[1], contours=dict(showlabels=True, labelfont=dict(size=12, color='white'))))
                        ...
                        for i, (col, year) in enumerate(zip([map_col1, map_col2], [year1, year2])):
                            with col:
                                ...
                                if data_year.empty:
                                    ...
                                    continue
                                fig = px.scatter_geo(data_year, lat='latitud_geo', lon='longitud_geo', color='precipitacion',
                                                     size='precipitacion', hover_name='nom_est', color_continuous_scale='YIGnBu',
...
with tab5:
    ...
    if analysis_mode == "Completar series (interpolación)":
        ...
        csv_completed = convert_df_to_csv(st.session_state.df_monthly_processed[st.session_state.df_monthly_processed['nom_est'].isin(selected_stations)])
        st.download_button("Descargar CSV con Series Completadas", csv_completed, 'precipitacion_mensual_completada.csv', 'text/csv', key='download-completado')
    else:
