...
def display_sidebar_controls():
    ...
    with st.sidebar.expander("**2. Selección de Estaciones y Período**", expanded=True):
        stations_filtered_for_ui = st.session_state.gdf_stations.copy()
        if selected_regions:
            stations_filtered_for_ui = stations_filtered_for_ui[stations_filtered_for_ui[Config.REGION_COL].isin(selected_regions)]
        if selected_municipios:
            stations_filtered_for_ui = stations_filtered_for_ui[stations_filtered_for_ui[Config.MUNICIPALITY_COL].isin(selected_municipios)]
        if selected_celdas:
            stations_filtered_for_ui = stations_filtered_for_ui[stations_filtered_for_ui[Config.CELL_COL].isin(selected_celdas)]
        
        stations_options = sorted(stations_filtered_for_ui[Config.STATION_NAME_COL].unique())
...
def get_filtered_data(min_data_perc, selected_altitudes, selected_regions, selected_municipios, selected_celdas, stations_for_analysis, year_range, meses_numeros, analysis_mode):
    ...
    stations_filtered_temp = st.session_state.gdf_stations.copy()
    
    stations_filtered_temp[Config.PERCENTAGE_COL] = pd.to_numeric(stations_filtered_temp[Config.PERCENTAGE_COL].astype(str).str.replace(',', '.'), errors='coerce').fillna(0)
    stations_filtered_temp = stations_filtered_temp[stations_filtered_temp[Config.PERCENTAGE_COL] >= min_data_perc]
    
    if selected_altitudes:
        conditions = []
        for r in selected_altitudes:
            if r == '0-500': conditions.append((stations_filtered_temp[Config.ALTITUDE_COL] >= 0) & (stations_filtered_temp[Config.ALTITUDE_COL] <= 500))
            elif r == '500-1000': conditions.append((stations_filtered_temp[Config.ALTITUDE_COL] > 500) & (stations_filtered_temp[Config.ALTITUDE_COL] <= 1000))
            elif r == '1000-2000': conditions.append((stations_filtered_temp[Config.ALTITUDE_COL] > 1000) & (stations_filtered_temp[Config.ALTITUDE_COL] <= 2000))
            elif r == '2000-3000': conditions.append((stations_filtered_temp[Config.ALTITUDE_COL] > 2000) & (stations_filtered_temp[Config.ALTITUDE_COL] <= 3000))
            elif r == '>3000': conditions.append(stations_filtered_temp[Config.ALTITUDE_COL] > 3000)
        if conditions: stations_filtered_temp = stations_filtered_temp[pd.concat(conditions, axis=1).any(axis=1)]

    if selected_regions:
        stations_filtered_temp = stations_filtered_temp[stations_filtered_temp[Config.REGION_COL].isin(selected_regions)]
    
    if selected_municipios:
        stations_filtered_temp = stations_filtered_temp[stations_filtered_temp[Config.MUNICIPALITY_COL].isin(selected_municipios)]

    if selected_celdas:
        stations_filtered_temp = stations_filtered_temp[stations_filtered_temp[Config.CELL_COL].isin(selected_celdas)]
        
    gdf_filtered = stations_filtered_temp[stations_filtered_temp[Config.STATION_NAME_COL].isin(stations_for_analysis)]
