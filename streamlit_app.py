...
with tendencias_tab:
    ...
    if len(stations_for_analysis) == 0:
        ...
        # Se agregan las pestañas para el gráfico y la tabla
        tendencia_graf_tab, tendencia_tabla_tab, pronostico_tab, prophet_tab = st.tabs(["Gráfico de Tendencia", "Tabla de Tendencias y Significancia", "Pronóstico SARIMA", "Pronóstico Prophet"])
 
        # Contenido de la pestaña "Gráfico de Tendencia" (lógica existente, pero con filtro mejorado)
        with tendencia_graf_tab:
            st.subheader("Tendencia de Precipitación Anual")
             
            analysis_type = st.radio("Tipo de Análisis de Tendencia:", ["Promedio de la selección", "Estación individual"], horizontal=True, key="trend_analysis_type")
            ...
            if df_to_analyze is not None and len(df_to_analyze) > 2:
                ...
                # Regresión lineal solo con valores > 0
                slope, intercept, _, p_value, _ = stats.linregress(df_to_analyze['año_num'], df_to_analyze['precipitacion'])
                ...
                st.warning("No hay suficientes datos de precipitación mayores a cero en el período seleccionado para calcular una tendencia.")

        # --- INICIO: NUEVA PESTAÑA PARA LA TABLA DE TENDENCIAS ---
        with tendencia_tabla_tab:
            st.subheader("Valores de Tendencia y Significancia por Estación")
            st.info("El cálculo de la tendencia se realiza solo con los valores de precipitación mayores a cero.")
            
            if len(stations_for_analysis) < 1:
                st.warning("Por favor, seleccione al menos una estación para ver los resultados.")
            else:
                tendencias_data = []
                # Filtra de antemano el DataFrame para solo incluir estaciones seleccionadas
                df_anual_filtered = df_anual_melted[df_anual_melted['nom_est'].isin(stations_for_analysis)]
                
                for station_name in stations_for_analysis:
                    df_station = df_anual_filtered[df_anual_filtered['nom_est'] == station_name].copy()
                    
                    # Aplicar el filtro de precipitación > 0
                    df_station = df_station[df_station['precipitacion'] > 0].copy()
                    
                    if len(df_station) > 2:
                        df_station['año_num'] = pd.to_numeric(df_station['año'])
                        slope, _, _, p_value, _ = stats.linregress(df_station['año_num'], df_station['precipitacion'])
                        
                        significancia_texto = "Sí" if p_value < 0.05 else "No"
                        
                        tendencias_data.append({
                            "Estación": station_name,
                            "Tendencia (mm/año)": round(slope, 2),
                        ...
                        tendencias_data.append({
                            "Estación": station_name,
                            "Tendencia (mm/año)": "N/A",
                            "Valor P": "N/A",
                            "Significancia": "No hay suficientes datos"
                        })
