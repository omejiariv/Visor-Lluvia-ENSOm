                ...
                with st.spinner("Generando mapa Kriging..."):
                    ...
                    if st.button("Calcular Precipitación por Municipio"):
                        with st.spinner("Calculando promedios por municipio..."):
                            ...
                            # Se asegura que la columna de nombres de municipio se mantenga
    ...
    with coropletico_tab:
        ...
        if st.session_state.gdf_municipal_stats is not None and not st.session_state.gdf_municipal_stats.empty:
            ...
            with map_col:
                ...
                # Aseguramos que la columna del nombre del municipio se llame 'nombre_mpio'
                # Esto asume que el GeoDataFrame cargado tiene una columna con los nombres de los municipios
                # y que el nombre de esa columna es 'nombre_mpio' o similar.
                municipio_col_name = next((col for col in gdf_municipios_data.columns if 'municipio' in col), None)
                if municipio_col_name:
                    data_for_choropleth = gdf_municipios_data[[municipio_col_name, 'promedio_precipitacion']]
                else:
                    st.warning("No se encontró la columna de nombres de municipio. El mapa no se puede generar.")
                    return
                ...
                folium.Choropleth(
                    ...
                    data=data_for_choropleth, columns=[municipio_col_name, 'promedio_precipitacion'],
                    key_on=f'feature.properties.{municipio_col_name}', fill_color='YlGnBu', fill_opacity=0.7,
