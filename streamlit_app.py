...
def preprocess_data(uploaded_file_mapa, uploaded_file_precip, uploaded_zip_shapefile, uploaded_file_soi, uploaded_file_iod):
    ...
    if uploaded_file_soi:
        df_soi = load_data(uploaded_file_soi, sep=';', header=0)
        if df_soi is not None:
            # Se asume que las columnas son año y 12 meses
            df_soi.columns = ['año', 'ene', 'feb', 'mar', 'abr', 'may', 'jun', 'jul', 'ago', 'sep', 'oct', 'nov', 'dic']
            # Se derriten las columnas de meses en una sola
            df_soi_melted = df_soi.melt(id_vars='año', var_name='mes', value_name='soi')
            # Se crea la columna de fecha para unirse
            df_soi_melted['mes_num'] = df_soi_melted['mes'].map({
                'ene': 1, 'feb': 2, 'mar': 3, 'abr': 4, 'may': 5, 'jun': 6,
                'jul': 7, 'ago': 8, 'sep': 9, 'oct': 10, 'nov': 11, 'dic': 12
            })
            df_soi_melted['fecha_mes_año'] = pd.to_datetime(df_soi_melted['año'].astype(str) + '-' + df_soi_melted['mes_num'].astype(str), format='%Y-%m')
            st.session_state.df_soi = df_soi_melted.copy()
            # Se eliminan las columnas auxiliares
            st.session_state.df_soi.drop(columns=['año', 'mes', 'mes_num'], inplace=True)
    
    if uploaded_file_iod:
        df_iod = load_data(uploaded_file_iod, sep=';', header=0)
        if df_iod is not None:
            # Se asume que las columnas son año y 12 meses
            df_iod.columns = ['año', 'ene', 'feb', 'mar', 'abr', 'may', 'jun', 'jul', 'ago', 'sep', 'oct', 'nov', 'dic']
            df_iod_melted = df_iod.melt(id_vars='año', var_name='mes', value_name='iod')
            meses_map = {'ene': 1, 'feb': 2, 'mar': 3, 'abr': 4, 'may': 5, 'jun': 6,
                         'jul': 7, 'ago': 8, 'sep': 9, 'oct': 10, 'nov': 11, 'dic': 12}
            df_iod_melted['mes'] = df_iod_melted['mes'].map(meses_map)
            ...
            # Se eliminan las columnas auxiliares
            st.session_state.df_iod.drop(columns=['año', 'mes', 'mes_num'], inplace=True)
    ...
    if not st.session_state.data_loaded:
        if not all([uploaded_file_mapa, uploaded_file_precip, uploaded_zip_shapefile]):
            ...
            with st.spinner("Procesando archivos y cargando datos... Esto puede tomar un momento."):
                ...
                if uploaded_file_soi:
                    st.session_state.df_soi = load_data(uploaded_file_soi, sep=',', header=None, lower_case=False)
                    if st.session_state.df_soi is not None:
                        st.session_state.df_soi.columns = ['año', 'ene', 'feb', 'mar', 'abr', 'may', 'jun', 'jul', 'ago', 'sep', 'oct', 'nov', 'dic']
                        ...
                        st.session_state.df_soi = df_soi_melted.copy()
                if uploaded_file_iod:
                    st.session_state.df_iod = load_data(uploaded_file_iod, sep=',', header=None, lower_case=False)
