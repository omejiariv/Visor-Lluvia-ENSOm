...
def display_stats_tab(df_long, df_anual_melted, df_monthly_filtered, stations_for_analysis, analysis_mode):
    ...
    with matriz_tab:
        ...
        if analysis_mode == "Completar series (interpolación)" and view_mode == "Número de Datos Completados":
            completed_data = df_monthly_filtered[(df_monthly_filtered[Config.ORIGIN_COL] == 'Completado')]
...
def get_filtered_data(min_data_perc, selected_altitudes, selected_regions, selected_municipios, selected_celdas, year_range, meses_numeros, analysis_mode):
    """Función central para aplicar todos los filtros y preprocesar los datos."""
    
    # 1. Filtrar estaciones
    stations_master_list = st.session_state.gdf_stations.copy()
    
    if Config.PERCENTAGE_COL in stations_master_list.columns:
        stations_master_list[Config.PERCENTAGE_COL] = pd.to_numeric(stations_master_list[Config.PERCENTAGE_COL].astype(str).str.replace(',', '.'), errors='coerce').fillna(0)
        stations_master_list = stations_master_list[stations_master_list[Config.PERCENTAGE_COL] >= min_data_perc]
    
    if selected_altitudes:
        conditions = []
        for r in selected_altitudes:
            if r == '0-500': conditions.append((stations_master_list[Config.ALTITUDE_COL] >= 0) & (stations_master_list[Config.ALTITUDE_COL] <= 500))
            elif r == '500-1000': conditions.append((stations_master_list[Config.ALTITUDE_COL] > 500) & (stations_master_list[Config.ALTITUDE_COL] <= 1000))
            elif r == '1000-2000': conditions.append((stations_master_list[Config.ALTITUDE_COL] > 1000) & (stations_master_list[Config.ALTITUDE_COL] <= 2000))
            elif r == '2000-3000': conditions.append((stations_master_list[Config.ALTITUDE_COL] > 2000) & (stations_master_list[Config.ALTITUDE_COL] <= 3000))
            elif r == '>3000': conditions.append(stations_master_list[Config.ALTITUDE_COL] > 3000)
        if conditions: stations_master_list = stations_master_list[pd.concat(conditions, axis=1).any(axis=1)]

    if selected_regions:
        stations_master_list = stations_master_list[stations_master_list[Config.REGION_COL].isin(selected_regions)]
    
    if selected_municipios:
        stations_master_list = stations_master_list[stations_master_list[Config.MUNICIPALITY_COL].isin(selected_municipios)]

    if selected_celdas:
        stations_master_list = stations_master_list[stations_master_list[Config.CELL_COL].isin(selected_celdas)]
        
    stations_options = sorted(stations_master_list[Config.STATION_NAME_COL].unique())
    
    # Manejar "Seleccionar todas"
    if 'select_all_checkbox' in st.session_state and st.session_state.select_all_checkbox:
        stations_for_analysis = stations_options
    else:
        stations_for_analysis = st.session_state.get('station_multiselect', [])
    
    if not stations_for_analysis:
        stations_for_analysis = stations_options
        
    gdf_filtered = stations_master_list[stations_master_list[Config.STATION_NAME_COL].isin(stations_for_analysis)]

    # 2. Procesamiento de datos mensuales
    df_long = st.session_state.df_long.copy()
    if analysis_mode == "Completar series (interpolación)":
        df_monthly_processed = complete_series(df_long)
    else:
        df_monthly_processed = df_long
    ...
    # 3. Procesamiento de datos anuales
    ...
    return gdf_filtered, df_anual_melted, df_monthly_filtered, stations_for_analysis, df_long, df_monthly_processed, stations_options
...
def main():
    ...
    with st.sidebar.expander("**1. Filtros Geográficos y de Datos**", expanded=True):
        ...
        temp_filtered_for_ui = st.session_state.gdf_stations.copy()
        if st.session_state.selected_altitudes:
            conditions = []
            for r in st.session_state.selected_altitudes:
                if r == '0-500': conditions.append((temp_filtered_for_ui[Config.ALTITUDE_COL] >= 0) & (temp_filtered_for_ui[Config.ALTITUDE_COL] <= 500))
                elif r == '500-1000': conditions.append((temp_filtered_for_ui[Config.ALTITUDE_COL] > 500) & (temp_filtered_for_ui[Config.ALTITUDE_COL] <= 1000))
                elif r == '1000-2000': conditions.append((temp_filtered_for_ui[Config.ALTITUDE_COL] > 1000) & (temp_filtered_for_ui[Config.ALTITUDE_COL] <= 2000))
                elif r == '2000-3000': conditions.append((temp_filtered_for_ui[Config.ALTITUDE_COL] > 2000) & (temp_filtered_for_ui[Config.ALTITUDE_COL] <= 3000))
                elif r == '>3000': conditions.append(temp_filtered_for_ui[Config.ALTITUDE_COL] > 3000)
            if conditions: temp_filtered_for_ui = temp_filtered_for_ui[pd.concat(conditions, axis=1).any(axis=1)]

        regions_list = sorted(temp_filtered_for_ui[Config.REGION_COL].dropna().unique())
        ...
        if selected_regions:
            temp_filtered_for_ui = temp_filtered_for_ui[temp_filtered_for_ui[Config.REGION_COL].isin(selected_regions)]
        ...
        if selected_municipios:
            temp_filtered_for_ui = temp_filtered_for_ui[temp_filtered_for_ui[Config.MUNICIPALITY_COL].isin(selected_municipios)]
    ...
    with st.sidebar.expander("**2. Selección de Estaciones y Período**", expanded=True):
        ...
        if st.session_state.selected_regions:
            stations_master_list = stations_master_list[stations_master_list[Config.REGION_COL].isin(st.session_state.selected_regions)]
        if st.session_state.selected_municipios:
            stations_master_list = stations_master_list[stations_master_list[Config.MUNICIPALITY_COL].isin(st.session_state.selected_municipios)]
    ...
    # --- Lógica centralizada de procesamiento de datos ---
    (
        gdf_filtered, 
        df_anual_melted, 
        df_monthly_filtered, 
    ...
    st.session_state.stations_for_analysis = stations_for_analysis
    st.session_state.df_anual_melted = df_anual_melted
    ...
    with bienvenida_tab:
        display_welcome_tab()
    with mapa_tab:
        display_spatial_distribution_tab(st.session_state.gdf_filtered, st.session_state.df_anual_melted, st.session_state.stations_for_analysis)
    with graficos_tab:
        display_graphs_tab(st.session_state.df_anual_melted, st.session_state.df_monthly_filtered, st.session_state.stations_for_analysis)
    with mapas_avanzados_tab:
        display_advanced_maps_tab(st.session_state.gdf_filtered, st.session_state.df_anual_melted, st.session_state.stations_for_analysis, st.session_state.df_monthly_filtered)
    with anomalias_tab:
        display_anomalies_tab(st.session_state.df_long, st.session_state.df_monthly_filtered, st.session_state.stations_for_analysis)
    with estadisticas_tab:
        display_stats_tab(st.session_state.df_long, st.session_state.df_anual_melted, st.session_state.df_monthly_filtered, st.session_state.stations_for_analysis, st.session_state.analysis_mode)
    with correlacion_tab:
        display_correlation_tab(st.session_state.df_monthly_filtered, st.session_state.stations_for_analysis)
    with enso_tab:
        display_enso_tab(st.session_state.df_monthly_filtered, st.session_state.df_enso, st.session_state.gdf_filtered, st.session_state.stations_for_analysis)
    with tendencias_tab:
        display_trends_and_forecast_tab(st.session_state.df_anual_melted, st.session_state.df_monthly_processed, st.session_state.stations_for_analysis)
    with descargas_tab:
        display_downloads_tab(st.session_state.df_anual_melted, st.session_state.df_monthly_filtered, st.session_state.stations_for_analysis, st.session_state.analysis_mode)
    with tabla_estaciones_tab:
        display_station_table_tab(st.session_state.gdf_filtered, st.session_state.df_anual_melted, st.session_state.stations_for_analysis)
